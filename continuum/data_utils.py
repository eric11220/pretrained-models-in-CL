...
class dataset_transform(data.Dataset):
    ...
    def __getitem__(self, idx):
        x = self.x[idx]
        if isinstance(x, str): # CUB200 where input is string
            x = self.loader(x)
        else: # x is np.array
            x = Image.fromarray(x)
...
def setup_test_loader(test_data, params):
    ...
    for (x_test, y_test) in test_data:
        test_dataset = dataset_transform(x_test, y_test,
                                         transform=transforms_match(model=params.model, reshape224=params.reshape224, test=True)[params.data])

        test_loader = data.DataLoader(test_dataset, batch_size=params.test_batch, shuffle=True, num_workers=4)
