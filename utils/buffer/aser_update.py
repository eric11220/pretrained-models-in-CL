...
class ASER_update(object):
    ...
    def _update_by_knn_sv(self, model, buffer, cur_x, cur_y):
        ...
        sv_matrix = compute_knn_sv(model, eval_x, eval_y, cand_x, cand_y, self.k, device=self.device, fix_bn=self.fix_bn)
        sv = sv_matrix.sum(0)
        ...
        buffer.buffer_img[ind_buffer] = x_upt.cpu()
        buffer.buffer_label[ind_buffer] = y_upt.cpu()
