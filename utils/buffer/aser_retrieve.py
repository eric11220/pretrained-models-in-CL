...
class ASER_retrieve(object):
    ...
    def retrieve(self, buffer, **kwargs):
        ...
        ret_x, ret_y = maybe_cuda(ret_x), maybe_cuda(ret_y)
        return ret_x, ret_y
    ...
    def _retrieve_by_knn_sv(self, model, buffer_x, buffer_y, cur_x, cur_y, num_retrieve):
        ...
        sv_matrix_adv = compute_knn_sv(model, eval_adv_x, eval_adv_y, cand_x, cand_y, self.k, device=self.device, fix_bn=self.fix_bn)
        ...
        if self.aser_type != "neg_sv":
            ...
            sv_matrix_coop = \
                compute_knn_sv(model, eval_coop_x, eval_coop_y, cand_x, cand_y, self.k, device=self.device, fix_bn=self.fix_bn)
