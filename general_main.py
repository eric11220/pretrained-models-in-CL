...
def main(args):
    ...
    if args.offline_lr is None:
        args.offline_lr = args.learning_rate
...
if __name__ == "__main__":
    ...
    ########################General#########################
    parser.add_argument('--num_runs', dest='num_runs', default=3, type=int,
    ...
    parser.add_argument('--cal-uniformity', action='store_true')
    ...
    parser.add_argument('--model', default='reduced_rn18', type=str,
                        choices=['pretrained_rn50', 'pretrained_rn18', 'rn18', 'clip', 'reduced_rn18',
                                 'simclr', 'swav', 'barlow_twins'],
                        help='Model for SCR')
    parser.add_argument('--exp_name', type=str, help='Experiment name')
    parser.add_argument('--reshape224', action='store_true', help='Reshape images to 224')
    parser.add_argument('--fix_bn', action='store_true', help='Fix batch normalization')
    parser.add_argument('--class_order', default=None, help='Class order')
    parser.add_argument('--feat_norm', action='store_true', help='Normalize feature to unit length')
    parser.add_argument('--store_checkpoint', action='store_true', help='Save intermediate checkpoint')
    parser.add_argument('--store_mem_snapshot', action='store_true', help='Save memory snapshot after each task')
    parser.add_argument('--linear_probe', action='store_true', help='Whether to just train the classifier for GDumb')
    parser.add_argument('--init', type=str, help='Checkpoint for model initialization')
    parser.add_argument('--offline_epoch', type=int, default=0, help='If > 0, Apply two-stage training after streaming')
    parser.add_argument('--offline_lr', type=float, default=None, help='LR in offline training')

    parser.add_argument('--target_run', type=int, default=None, help='Target run #')
